...
var editToolVersionComponentControllerApp = angular.module('editToolVersionComponentControllerApp', []);

editToolVersionComponentControllerApp.controller('EditToolVersionController', function ($scope, $state, $rootScope, $window, GetAllTools, CreateTool, toolFileUpload, $timeout, PrerequisitesViewAll, TagsAll, GetAllDeploymentPlugins) {
    $rootScope.versionData = $rootScope.toolDetailsFactory.getVersionList();
    $rootScope.mediaFilesOriginalLength = [];
    for(var i=0; i<$rootScope.versionData.length; i++)
    {
        if($rootScope.versionData[i].media_file)
        {
            $rootScope.mediaFilesOriginalLength.push({'version_number' : $rootScope.versionData[i].version_number, 'length' : $rootScope.versionData[i].media_file.media_files.length});
        }
    }
    ...
    $scope.mediaFilesOriginalLength = [];
    ...
    $scope.prerequisites = [];
    $scope.DependentTools = [];
    // fuction to close the all opened drop down
    ...
    function(toolAllSuccessResponse,headers)
    {
        $scope.applications = toolAllSuccessResponse.data.data;
        for(var t1=0; t1<toolAllSuccessResponse.data.data.length; t1++)
        {
            for(var t2=0; t2<toolAllSuccessResponse.data.data[t1].versions.length; t2++)
            {
                $scope.DependentTools.push({'tool_id' : toolAllSuccessResponse.data.data[t1]._id.$oid, 'tool_name' : toolAllSuccessResponse.data.data[t1].name, 'version_id' : toolAllSuccessResponse.data.data[t1].versions[t2].version_id, 'version_name' : toolAllSuccessResponse.data.data[t1].versions[t2].version_name, 'version_number' : toolAllSuccessResponse.data.data[t1].versions[t2].version_number});
            }
        }
    },
    function(toolAllErrorResponse)
    {
        $rootScope.handleResponse(toolAllErrorResponse);
    });
    ...
    $scope.isCertifiedSelected = function(certificate, versionIndex)
    {
        var flag = 0;
        if($rootScope.versionData[versionIndex].mps_certified)
        {
            for(var j=0; j<$rootScope.versionData[versionIndex].mps_certified.length; j++)
        {
            if(certificate === $rootScope.versionData[versionIndex].mps_certified[j])
            {
                flag++;
            }
        }
    ...
    {
        $rootScope.versionData[versionIndex].mps_certified = [];
    }

    };

    $scope.isPrerequisiteSelected = function(prerequisite, versionIndex)
    {
        var flag = 0;
        if($rootScope.versionData[versionIndex].pre_requiests)
        {
            for(var j=0; j<$rootScope.versionData[versionIndex].pre_requiests.length; j++)
            {
                if((prerequisite.name === $rootScope.versionData[versionIndex].pre_requiests[j].prerequisites_name) && (prerequisite.version === $rootScope.versionData[versionIndex].pre_requiests[j].version))
                {
                    flag++;
                }
            }
    ...
    $scope.showSettings = function(version_index)
    {
        if(document.getElementById("add_settings").style.display === "none" || document.getElementById("add_settings").style.display === "")
        {
            ...
            $scope.addNewField(version_index);
        }
        else
        {
    ...
    $scope.changeMandatoryTag = function(version, fieldIndex)
    {
        for(var ind=0; ind<$rootScope.versionData.length; ind++)
        {
            if(version === $rootScope.versionData[ind].version_number)
            {
                if($rootScope.versionData[ind].deployment_field.fields[fieldIndex].is_mandatory)
                {
                    if($rootScope.versionData[ind].deployment_field.fields[fieldIndex].is_mandatory === true || $rootScope.versionData[ind].deployment_field.fields[fieldIndex].is_mandatory === 'true')
                    {
                        $rootScope.versionData[ind].deployment_field.fields[fieldIndex].is_mandatory = false;
                    }
                    else
                    {
                        $rootScope.versionData[ind].deployment_field.fields[fieldIndex].is_mandatory = true;
                    }
                }
                else
                {
    ...
    $scope.showEditSettings = function(index, version_index)
    {
    ...
    $scope.showCertified = function(version_index, index)
    {
    ...
    $scope.resetValidation = function(version, attribute)
    {
        $scope.version_errors.version_name = "";
        $scope.version_errors.branch_tag = "";
        for(var br=0; br<$rootScope.versionData.length; br++)
        {
            if(version === $rootScope.versionData[br].version_number)
            {
                $rootScope.versionData[br].branch_tag = attribute;
            }
        }
    };
    $scope.closeSettings = function(vIndex, fieldIndex)
    {
        ...
        {
            $scope.removeField(vIndex, fieldIndex);
        }
        $('#add_settings').hide(700);
    };
    ...
    $scope.isFieldSelected = function(fieldName, fieldValue)
    {
        var duIndex = 0;
        var versionIndex =0;
        for(var b=0;  b<$rootScope.versionData.length; b++)
        {
            versionIndex =b;
            for(var d=0;  d<$rootScope.versionData[b].deployment_field.fields.length; d++)
            {
                if($rootScope.versionData[b].deployment_field.fields[d].input_name === fieldName)
                {
                    duIndex = d;
                }
            }
        ...
        if($rootScope.versionData[versionIndex].deployment_field.fields[duIndex].default_value)
        {
            var result = $.inArray(fieldValue, $rootScope.versionData[versionIndex].deployment_field.fields[duIndex].default_value);
            if(result < 0)
            {
                return false;
            }
    ...
    $scope.addDefaultValueToCheckbox = function(fieldName, fieldValue)
    {
        ...
        {
            versionIndex =b;
            for(var validFieldIndex = 0; validFieldIndex<$rootScope.versionData[b].deployment_field.fields.length; validFieldIndex++)
            {
                if($rootScope.versionData[b].deployment_field.fields[validFieldIndex].input_name === fieldName)
                {
        ...
        if($rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value)
        {
            var result = $.inArray(fieldValue, $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value);
            if(result < 0)
            {
               $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value.push(fieldValue);
            }
            else
            {
                var index = $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value.indexOf(fieldValue);
                $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value.splice(index, 1);
            }
        }
        else
        {
            $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value = [];
            $rootScope.versionData[versionIndex].deployment_field.fields[fieldIndex].default_value.push(fieldValue);
        }
    };

    $scope.showEditPrerequisites = function(version, index)
    {
        for(var vInd=0; vInd<$rootScope.versionData.length; vInd++)
        {
            if($rootScope.versionData[vInd].version_number === version)
            {
                if($rootScope.versionData[vInd].document)
                {
                    $rootScope.vIndex = vInd;
                }
            }
        }
    ...
    $scope.showDependentTools = function(version, index)
    {
        for(var vInd=0; vInd<$rootScope.versionData.length; vInd++)
        {
            if($rootScope.versionData[vInd].version_number === version)
    ...
    $scope.closeEditLogo = function()
    {
        ...
        srcId.src = $scope.application.data.thumbnail_logo;
        $scope.clearLogo();
    };
    ...
    $scope.selectMPSCertification = function(certificate, version_index)
    {
        var mps_flag = 0;
        var ind = 0;
        if($rootScope.versionData[version_index].mps_certified)
        {
            for(var c=0;  c<$rootScope.versionData[version_index].mps_certified.length; c++)
            {
                if(certificate===$rootScope.versionData[version_index].mps_certified[c])
                {
                    mps_flag++;
                    ind = c;
                }
            ...
            if(mps_flag===0)
            {
                $rootScope.versionData[version_index].mps_certified.push(certificate);
            }
            else
            {
                $rootScope.versionData[version_index].mps_certified.splice(ind, 1);
            }

        }
        else
        {
            $rootScope.versionData[version_index].mps_certified = [];
            $rootScope.versionData[version_index].mps_certified.push(certificate);
        }
    };

    $scope.selectPrerequisite = function(prerequisite, version_index)
    {
        var prerequisite_flag = 0;
        var ind = 0;
        if($rootScope.versionData[version_index].pre_requiests)
        {
            for(var c=0;  c<$rootScope.versionData[version_index].pre_requiests.length; c++)
            {
                if((prerequisite.prerequisites_name===$rootScope.versionData[version_index].pre_requiests[c].name) && (prerequisite.version===$rootScope.versionData[version_index].pre_requiests[c].version))
                {
                    prerequisite_flag++;
                    ind = c;
                }
            ...
            {
                $rootScope.versionData[version_index].pre_requiests.push({'name' : prerequisite.prerequisites_name, 'version' : prerequisite.version});
            }
            else
            {
                $rootScope.versionData[version_index].pre_requiests.splice(ind, 1);
            }

        }
        else
        {
            $rootScope.versionData[version_index].pre_requiests = [];
            $rootScope.versionData[version_index].pre_requiests.push({'name' : prerequisite.prerequisites_name, 'version' : prerequisite.version});
        }
    };
    ...
    $scope.showEditDocument = function(version_index, doc_index)
    {
    ...
    $scope.closeDocument = function(vIndex)
    {
        var docIndex = $scope.docIndex;
        var url = $rootScope.versionData[0].document.documents[docIndex].url;
        if($scope.versionData[vIndex].document.documents[docIndex].name===null || $scope.versionData[vIndex].document.documents[docIndex].name==='' || $scope.versionData[vIndex].document.documents[docIndex].name===undefined)
        {
            $scope.version_errors.document_errors.name = 'Please enter document name!';
        }
        if($scope.versionData[vIndex].document.documents[docIndex].url===null || $scope.versionData[vIndex].document.documents[docIndex].url==='' || $scope.versionData[vIndex].document.documents[docIndex].url===undefined)
        {
            $scope.version_errors.document_errors.url = 'Please enter document url!';
        }
        if($rootScope.versionData[vIndex].document.documents[docIndex].url!=='' && $rootScope.versionData[vIndex].document.documents[docIndex].name==='')
        {
            $scope.version_errors.document_errors.name = 'Please enter document name!';
        }
        else if($rootScope.versionData[vIndex].document.documents[docIndex].name!=='' && $rootScope.versionData[vIndex].document.documents[docIndex].type==='')
        {
            $scope.version_errors.document_errors.type = 'Please enter document type!';
        }
        else if($rootScope.versionData[vIndex].document.documents[docIndex].name!=='' && $rootScope.versionData[vIndex].document.documents[docIndex].url==='')
        {
            $scope.version_errors.document_errors.url = 'Please enter document url!';
        }
    ...
    {
        $('#add_document').hide(700);
        //$scope.removeDocument(vIndex, document_index);
    };

    $scope.removeDocument = function(version_index, doc_index)
    {
        $scope.docIndex = doc_index;
        $scope.removeDocField( $scope.docIndex, $rootScope.vIndex);
    };
    ...
    $scope.addNewField = function(version_index)
    {
        if($rootScope.versionData[version_index].deployment_field===null)
        {
            $rootScope.versionData[version_index].deployment_field = {
                fields : []
            };
            $rootScope.fieldIndex = $rootScope.versionData[version_index].deployment_field.fields.length;
        }
        else
        {
            $rootScope.fieldIndex = $rootScope.versionData[version_index].deployment_field.fields.length;
            $rootScope.versionData[version_index].deployment_field.fields.push({
                order_id : $rootScope.fieldIndex.fieldIndex,
                is_existing : false
            });
        }
    };
    ...
    $scope.removeField = function(vIndex, fieldIndex)
    {
        $rootScope.versionData[vIndex].deployment_field.fields.splice(fieldIndex ,1);
    };
    ...
    $scope.removeThisField = function(index, version_number)
    {
        for(var version=0; version<$rootScope.versionData.length; version++)
        {
            ...
            {
                $rootScope.versionData[version].deployment_field.fields.splice(index, 1);
            }
        }
    };
    ...
    $scope.removeMediaFile = function(version, ind)
    {
        for(var versionIndex=0;versionIndex<$rootScope.versionData.length;versionIndex++)
        {
            if(version===$rootScope.versionData[versionIndex]._id.$oid)
            {
                $rootScope.versionData[versionIndex].media_file.media_files.splice(ind, 1);
            }
        }
    };
});
