...
var addNewDUSetControllerApp = angular.module('addNewDUSetControllerApp', ['ui.router','720kb.tooltips', 'deploymentUnitsServicesApp', 'prerequisitesServicesApp', 'authenticationServicesApp','tagsServicesApp','flexAttributeDirectiveApp']);

addNewDUSetControllerApp.controller('CreateNewDUSetController', function ($scope, $rootScope, $state, $cookieStore, PrerequisitesViewAll, ApprovalStatusAll, DUTypesAll, UserDetails, CreateDUSet, DeploymentUnitAll, TagsAll, duLogoFileUpload, DUSetAll) {
    var date = new Date();
    $scope.addDUFlag = 0;
   ...
   $scope.showEditLogo = function()
    ...
    $scope.cancelDUSetList = function()
    {
        document.getElementById("show_du_list").style.display = "none";
        $scope.selectedDU = [];
        if($scope.duList.length>0)
        {
            angular.copy($scope.duList, $scope.selectedDU);
        }

        if($scope.selectedDUToArrange.length>0)
        {
            for(var a=0; a<$scope.selectedDUToArrange.length; a++)
            {
                var duFlag_1 = 0;
                var duInd_1 = 0;
                for(var b=0; b<$scope.selectedDU.length; b++)
                {
                    if($scope.selectedDUToArrange[a].du_id !== $scope.selectedDU[b].du_id)
                    {
                        duFlag_1++;
                        duInd_1 = a;
                    }
                }
                if(duFlag_1 === $scope.selectedDU.length)
                {
                    $scope.selectedDU.push($scope.selectedDUToArrange[duInd_1]);
                }
            }
        }
    };
    ...
    $scope.deleteDUSetOrderList = function()
    {
        ...
        if($scope.duList.length < $scope.selectedDU.length)
        {
            for(var a=0; a<$scope.duList.length; a++)
            {
                ...
                if(duFlag_1 <1)
                {
                    $scope.selectedDU.push($scope.duList[a]);
                }
            }
   ...
   $scope.createNewDUSet = function(form)
   {
        var duSetData = {
            du_set : []
        };
...
//      Validation end here..
        ...
        CreateDUSet.save(duSetData, function(duSetCreateSuccessResponse)
        {
            var file = $scope.logo;
            if(file)
            {   file.duset_id= duSetCreateSuccessResponse.data._id;
                var uploadUrl = "/deploymentunitset/upload/logo";
                duLogoFileUpload.uploadDuSetLogoFileToUrl(file, uploadUrl);
            }
            $state.go('duDashboard');
            $rootScope.handleResponse(duSetCreateSuccessResponse);
        },
        function(duSetCreateErrorResponse)
        {
            $rootScope.handleResponse(duSetCreateErrorResponse);
        });
   };
